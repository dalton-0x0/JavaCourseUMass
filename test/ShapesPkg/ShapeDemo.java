...
public class ShapeDemo
{
    ...
    // as parameters.
    public static void sortArray(Shape[] shapes, int numShapes) // method to 
            //sort the array. Code derived from Week 5 assignment logic.
    {
        ...
        do
        {
            ...
            {
                if (shapes[i].area() > shapes[i+1].area()) //compare sizes
                {
                    Shape temp = shapes[i]; //assign greater to temp variable
                    shapes[i] = shapes[i+1]; //assign smaller to current index
                    shapes[i+1] = temp; //assign greater (in temp) to next index
                    swapped = true;
                }
            }
